...
def submit_to_deadline(plugin_details, job_details):
    LOGGER = get_logger()
    # Define the path to the Deadline Command executable
    deadline_cmd = "/opt/Thinkbox/Deadline10/bin/deadlinecommand"
    print ("*****************************dedline caleddd")
    # Write job and plugin information to temporary files
    # if job_details is ["brahmos", "bolt"]:

    job_file = write_job_info_file(str(job_details).strip())
    plugin_file = write_plugin_info_file(str(plugin_details).strip())
    # print ("****submit_to_deadline plugin_details",plugin_details)
    # print ("****submit_to_deadline job_details",job_details)
    ...
    allowlist_match = re.search(r"Allowlist=(.*)", job_details)
    ...
    if stdout:
        LOGGER.info(
            f"Source path: {args.source}, Machine Name: {socket.gethostname()}, User Name: {os.getlogin()}, Render blade: {allowlist}, Submitted successfully"
        )
    ...
    if stderr:
        ...
        LOGGER.error(
            f"ERROR - Source path: {args.source}, Machine Name: {socket.gethostname()}, User Name: {os.getlogin()}, Render blade: {allowlist}, Submitted Failed, \nError: {stderr_message}"
        )
...
def validate_source_path(source_config):
...
def get_source_and_destination():
    LOGGER = get_logger()
    destination_location = args.destination.lower()
    source_path = args.source
    source_location, source_config = get_source_config()
    validate_source_path(source_config)
    ...
    if destination_location in config:
        ...
        if server_name not in destination_config["server"]:
            LOGGER.error(
                f"ERROR - Invalid server name. Available options for '{args.destination}' are {', '.join(destination_config['server'].keys())}, but given server name is {server_name}"
            )
            raise ValueError(
        ...
        LOGGER.error(
            f"ERROR - Invalid destination. Available options are {', '.join(config.keys())}, but given destination is {args.destination}"
        )
        raise ValueError(
...
def get_info_txt():
    ...
    allowlist, rsync_arguments = get_source_and_destination()
...
def deadline_ingest(publish_data,server):
    src = publish_data['sg_filepath']
    dst=server
    print("55555555555555555555555555555",src)
    print("11111111111111 server",server)
    os.chdir(
    Path(__file__).resolve().parent
    )  # Change directory from symlink directory to project directory


    src, dst = get_info_txt()
    # shots.create(args.project, args.sequence, args.shot, args.dept, args.config)
    if dst =='brahmos':
        dst="chennai"
        submit_to_deadline(src,dst)
    # # else:
